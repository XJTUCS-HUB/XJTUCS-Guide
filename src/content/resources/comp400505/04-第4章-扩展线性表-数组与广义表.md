...
- **二维数组** `A[m][n]`推广到一般情况：
 **以行为主序**：`LOC(a_i,j) = LOC(a_0,0) + (i * n + j) * L`
 **以列为主序**：`LOC(a_i,j) = LOC(a_0,0) + (j * m + i) * L`
...
==上一章 [第3章 受限线性表：栈，队列和串](../03-第3章-受限线性表-栈-队列和串/)
下一章 [第5章 树与二叉树](../05-第5章-树与二叉树/)==
